...
import { PUTER_WORKER_URL, buildPuterWorkerUrl } from "./constants";
...
export const createProject = async ({ item, visibility="private" }: CreateProjectParams): Promise<DesignItem | null | undefined> => {
    ...
    try {
        const response = await puter.workers.exec(buildPuterWorkerUrl('/api/projects/save'), { 
            method: 'POST',
            body: JSON.stringify({
...
export const getProject = async () => {
    ...
    try {
        const response = await puter.workers.exec(buildPuterWorkerUrl('/api/projects/list'), { method: 'GET' });
...
export const getProjectById = async ({ id }: { id: string }) => {
    ...
    try {
        const response = await puter.workers.exec(
            buildPuterWorkerUrl(`/api/projects/get?id=${encodeURIComponent(id)}`),
            { method: "GET" },
        );
